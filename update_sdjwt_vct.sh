...
get_token() {
  curl -s -k -X POST "$KEYCLOAK_ADMIN_ADDR/realms/master/protocol/openid-connect/token" \
...
flow=$(curl -s -k -H "Authorization: Bearer $TOKEN" "$KEYCLOAK_ADMIN_ADDR/admin/realms/$KEYCLOAK_REALM/authentication/flows" | jq -c ".[] | select(.alias==\"$FLOW_ALIAS\")")
if [ -z "$flow" ]; then
  echo "Flow '$FLOW_ALIAS' not found in realm '$KEYCLOAK_REALM'. Skipping." >&2
  exit 0
fi

# URL-encode flow alias using jq
FLOW_ALIAS_ENC=$(printf '%s' "$FLOW_ALIAS" | jq -sRr @uri)

executions=$(curl -s -k -H "Authorization: Bearer $TOKEN" "$KEYCLOAK_ADMIN_ADDR/admin/realms/$KEYCLOAK_REALM/authentication/flows/$FLOW_ALIAS_ENC/executions")
...
if [ "$cfg_id" != "null" ] && [ -n "$cfg_id" ]; then
  ...
  cfg=$(curl -s -k -H "Authorization: Bearer $TOKEN" "$KEYCLOAK_ADMIN_ADDR/admin/realms/$KEYCLOAK_REALM/authentication/config/$cfg_id")
  ...
  # Create new config on execution (provide default fields expected by UI)
  body=$(jq -n \
