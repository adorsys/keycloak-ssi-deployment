...
- Admin console available at `${KEYCLOAK_ADMIN_ADDR}` (defaults to `https://localhost:8443`).

### Stop / Clean
```bash
keycloak-ssi compose down    # stop containers
...
keycloak-ssi help
```
Installs into `~/.local/bin` and symlinks project files under `~/.local/share/keycloak-ssi-deployment`.

## How It Works

```
+-------------------+     +------------------------+
| keycloak-ssi CLI  |---->| src/deployment/*.sh    |--> Keycloak (oid4vc)
| - setup/config    |     | src/credentials/*.sh   |
| - compose bridge  |     |                        |
+-------------------+     +------------------------+
         ^                        ^
         | config.yaml            | overrides/env vars
         +------------------------+
```
...
- `database.*`: container DB credentials/ports if you reuse Postgres elsewhere.
- `users.*`: passwords for demo users (never ship defaults to production).

## Operating the CLI

| Command | What it does |
| --- | --- |
| `setup [-d]` | Builds / starts Keycloak with TLS using scripts under `src/deployment/0.*`. Pass `-d` to detach logging to `target/keycloak.log`. |
| `compose <args>` | Convenience wrapper for `docker compose` using the provided file and config expansion. |
| `config` | Runs `1.oid4vci_test_deployment.sh` and `2.configure_user_4_account_client.sh` to create realm, credential client scopes, and demo identities. Requires Keycloak to be reachable at `${KEYCLOAK_ADMIN_ADDR}`. |
| `import` | Uses Keycloak Config CLI to import a ready-made realm (needs Java 21). |
| `test <preauth|authcode> <CredentialType>` | Invokes curl-based flows inside `src/credentials/`. |
| `stop` | Stops a tarball-based server started via `setup`. |
| `install / uninstall` | Manage global availability of the CLI. |

Use `keycloak-ssi help` for the built-in usage text (also mirrored in `docs/CLI_GUIDE.md`).
...
4. Requests IdentityCredential, SteuerberaterCredential, and KMACredential with key binding.
...
- **Keycloak never becomes ready**: check `target/keycloak.log`, ensure ports (HTTPS / DB) are free, and confirm `openssl`, `keytool`, and Java are installed.
...
## More Docs
- `src/` scripts - small, readable shell scripts for setup, configuration, and credential testing if you want to customize behavior.

That's it - a focused kit to light up OID4VC on Keycloak without extra ceremony.
