...
- Ready-made credential definitions (IdentityCredential, SteuerberaterCredential, KMACredential) and demo users for both pre-authorized and authorization-code flows.
...
- Logs land in `target/keycloak.log`; admin console available at `${KEYCLOAK_ADMIN_ADDR}` (defaults to `https://localhost:8443`).
...
| `config.override.yaml` | Optional environment-specific overrides checked into `.gitignore`. |
...
## More Docs
- `docs/CLI_GUIDE.md` - extended CLI reference and command list.
