...
# This script creates/updates the openid4vc-rest-api-jwt2 client
...
log_message "Checking if JWT client 'openid4vc-rest-api-jwt2' already exists..."
CLIENT_ID=$($KC_INSTALL_DIR/bin/kcadm.sh get clients -r "$KEYCLOAK_REALM" --fields id,clientId | jq -r '.[] | select(.clientId=="openid4vc-rest-api-jwt2") | .id')

if [ -z "$CLIENT_ID" ] || [ "$CLIENT_ID" == "null" ]; then
    log_message "Creating new JWT client 'openid4vc-rest-api-jwt2'..."
    cat "$SCRIPT_DIR/openid4vc-rest-api-jwt2.json" | $KC_INSTALL_DIR/bin/kcadm.sh create clients -r "$KEYCLOAK_REALM" -o -f - || exit_with_error "Failed to create JWT client"
    CLIENT_ID=$($KC_INSTALL_DIR/bin/kcadm.sh get clients -r "$KEYCLOAK_REALM" --fields id,clientId | jq -r '.[] | select(.clientId=="openid4vc-rest-api-jwt2") | .id')
    log_message "JWT client created successfully."
else
    log_message "JWT client 'openid4vc-rest-api-jwt2' already exists. Updating..."
    cat "$SCRIPT_DIR/openid4vc-rest-api-jwt2.json" | $KC_INSTALL_DIR/bin/kcadm.sh update clients/$CLIENT_ID -r "$KEYCLOAK_REALM" -o -f - || exit_with_error "Failed to update JWT client"
...
log_message "1. Client ID: openid4vc-rest-api-jwt2"
