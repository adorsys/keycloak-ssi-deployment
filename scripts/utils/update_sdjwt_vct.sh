...
executions=$(curl -s -k -H "Authorization: Bearer $TOKEN" \
  "$KEYCLOAK_URL/admin/realms/$KEYCLOAK_REALM/authentication/flows/$FLOW_ALIAS_ENC/executions")
...
if [ "$cfg_id" != "null" ] && [ -n "$cfg_id" ]; then
  ...
  cfg=$(curl -s -k -H "Authorization: Bearer $TOKEN" \
    "$KEYCLOAK_URL/admin/realms/$KEYCLOAK_REALM/authentication/config/$cfg_id")
  new_cfg=$(echo "$cfg" | jq --arg VCT "$VCT" '.config.vct = $VCT')
  curl -s -k -X PUT "$KEYCLOAK_URL/admin/realms/$KEYCLOAK_REALM/authentication/config/$cfg_id" \
  ...
  curl -s -k -X POST "$KEYCLOAK_URL/admin/realms/$KEYCLOAK_REALM/authentication/executions/$exec_id/config" \
    -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d "$body" > /dev/null
