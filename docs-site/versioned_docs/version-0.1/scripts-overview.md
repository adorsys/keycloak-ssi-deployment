...
## [0.start-kc-oid4vci.sh](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/0.start-kc-oid4vci.sh)
...
**Key functionalities:**
-   **Helper Script:** Sources common helper functions and environment variables from [`src/utils/helper.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/utils/helper.sh).
-   **Keycloak Shutdown:** Checks for and shuts down any running Keycloak instances to prevent conflicts.
-   **Keycloak Setup:** Executes [`src/deployment/setup-kc-oid4vci.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/deployment/setup-kc-oid4vci.sh) to download, unpack, and prepare Keycloak.
...
## [1.oid4vci_test_deployment.sh](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/1.oid4vci_test_deployment.sh)
...
-   **SD-JWT VCT Update:** Ensures the `sd-jwt` authenticator VCT is configured using [`src/utils/update_sdjwt_vct.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/utils/update_sdjwt_vct.sh).
...
## [2.configure_user_4_account_client.sh](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/deployment/2.configure_user_4_account_client.sh)

This new script is responsible for configuring a user (Francis) and enabling direct access grants for the `openid4vc-rest-api` client within Keycloak.

**Key functionalities:**
-   **Helper Script:** Sources common helper functions and environment variables from [`src/utils/helper.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/utils/helper.sh).
-   **Admin Token Acquisition:** Obtains an admin token for Keycloak configuration.
-   **Client Property Reading:** Reads the direct access property of the `openid4vc-rest-api` client.
-   **Client ID Storage:** Stores the `openid4vc-rest-api` client ID in an environment variable.
-   **Direct Grant Enablement:** Enables direct access grants on the `openid4vc-rest-api` client.
-   **User Creation:** Creates the user 'Francis' if they do not already exist.
-   **Password Setting:** Sets the password for the user 'Francis'.
-   **User Key Proof Preparation:** Generates a user key pair and proof header if they do not exist, using [`src/utils/crypto/generate_user_key.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/utils/crypto/generate_user_key.sh).

## [3.request_credentials_with_auth_code_flow.sh](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/3.request_credentials_with_auth_code_flow.sh)
...
-   **Key Proof Generation:** Generates a user key proof if it doesn't exist using [`src/utils/crypto/generate_key_proof.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/utils/crypto/generate_key_proof.sh).
...
## [3.retrieve_IdentityCredential.sh](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/3.retrieve_IdentityCredential.sh)
...
-   **Key Proof Generation:** Generates a user key proof using [`src/utils/crypto/generate_key_proof.sh`](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/src/utils/crypto/generate_key_proof.sh).
-   **Credential Request:** Prepares a request payload and obtains the `IdentityCredential` from the Keycloak credential endpoint.

## [3.retrieve_KMACredential.sh](https://github.com/adorsys/keycloak-ssi-deployment/blob/main/3.retrieve_KMACredential.sh)
